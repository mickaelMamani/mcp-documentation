...
- **Déclencheurs** : **au démarrage, et seulement au démarrage** (ADR #25). Une évolution du corpus est prise en compte par un redémarrage du service, qui refait un `fetch` de la référence configurée. `FileSystemWatcher`, tool `reindex`, endpoint `POST /admin/reindex` et polling git restent des évolutions possibles : la mécanique de swap ci-dessus les accepte sans changement, seul le déclencheur manque.
...
- Le commit effectivement servi est journalisé et exposé par `/health` (`docsRevision`) : c'est le seul moyen de vérifier qu'un redémarrage a bien pris la nouvelle documentation.
