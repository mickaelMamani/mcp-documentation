...
|---|---|---|
| `Title`, `Breadcrumb` | Custom | `WhitespaceTokenizer` → `WordDelimiterFilter` (camelCase, `/`, `_`, `-`, `{}`) → `LowerCaseFilter` → `ASCIIFoldingFilter`. Conserve aussi le token original (`Plug` et `plug`) |
...
| `Keywords` | Custom neutre | `WhitespaceTokenizer` → `LowerCaseFilter` → `ASCIIFoldingFilter`. **Pas de stemming** (champ bilingue) |
...
5. `k` = 30 candidats en v1 (top-N direct), 50 en v2 (avant reranking).
...
1. **Retrieval pur** (test xUnit, rapide, à chaque commit) : la question est reformulée par une table de correspondance FR→EN fixe (simule le LLM), puis `search_endpoints` est appelé directement. Mesures : rang du premier `expected`, MRR, hit@3, tokens renvoyés. Seuils = objectifs §1 ; régression = échec de build.
